...
golang_import_
    : 'go_import' golang_alias? go_package LITERAL
    ;
...
struct_
    : 'struct' IDENTIFIER struct_hint? extends? '{' field* '}' type_annotations?
    ;
...
extends
    : 'extends' point? 'struct' IDENTIFIER
    ;
...
post_
    : method_description? 'POST' url_ method_type method_type_hint? IDENTIFIER '(' method_param_? ')' not_login?  list_separator?
    ;

get_
    : method_description? 'GET' url_ method_type method_type_hint? IDENTIFIER '(' get_param_? ')' not_login?  list_separator?
    ;

put_
    : method_description? 'PUT' url_ method_type method_type_hint? IDENTIFIER '(' method_param_? ')' not_login?  list_separator?
    ;
...
simple_param_
    : field_req? real_base_type_parm field_annotations?
    | field_req? real_base_type_list_parm field_annotations?
    ;
real_base_type_parm
...
real_base_type
    :  TYPE_BOOL | TYPE_BYTE | TYPE_I8 | TYPE_I16 | TYPE_I32 | TYPE_I64 | TYPE_FLOAT | TYPE_DOUBLE | TYPE_STRING
    ;
